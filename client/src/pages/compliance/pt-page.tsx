...
import { FileSpreadsheet, FileText, Download, Upload, Calculator, Building2, Users, Calendar } from "lucide-react";
...
export default function PtPage() {
  ...
  const getReportPeriod = () => {
    ...
    if (selectedPeriod === "day") {
      const dateString = selectedMonth === new Date().getMonth() && selectedYear === new Date().getFullYear() ? new Date().toISOString().split('T')[0] : `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}-01`;
  ...
  const generateReport = async () => {
    ...
    await addCompanyHeader(doc, { 
      ...
      subtitle: `For the Month of ${monthsList[selectedMonth]} ${selectedYear}`
    });
    addFooter(doc);
  ...
  const handleExportExcel = () => {
    const flatData = Object.values(ptData).flatMap(depts => Object.values(depts).flat());
    exportToExcel(flatData, `PT_Report_${monthsList[selectedMonth]}_${selectedYear}`);
    toast({ title: "Export Successful", description: "Excel report has been downloaded." });
  };

  const handleExportTxt = () => {
    const flatData = Object.values(ptData).flatMap(depts => Object.values(depts).flat());
    exportToTxt(flatData, `PT_Report_${monthsList[selectedMonth]}_${selectedYear}`, "Professional Tax Report");
